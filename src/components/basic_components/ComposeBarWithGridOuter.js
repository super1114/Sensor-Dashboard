...
import styles from './ComposeBarWithGridOuter.module.scss'

const ComposeBarWithGridOuter = ({ data, height = 30, rowHeight = RowHeight, width = 400 }) => {
  if (!data) {
    return null
  }

  const maxSum = Math.max(...data.map(row => sumUpArray(row)))
          ...
          return (
            ...
            >
              ...
              {row.map((cell, idx) => {
                return (
                  <div
                    className={clsx(styles.composeGridBarCellText, {
                      [styles.blue]: idx === 0,
                      [styles.cyan]: idx === 1,
                      [styles.purple]: idx === 2,
                      [styles.green]: idx === 3,
...
export default ComposeBarWithGridOuter
